...
export default function PDFToImagePage() {
  ...
  function reset() { setFile(null); setDone(false); setError(null); setMultiResult(null); }

  async function handleConvert() {
    if (!file) { setError('Select a PDF file.'); return; }
    setLoading(true); setError(null); setMultiResult(null);
    try {
      const result = await pdfToImage(file, format, dpi);
      if (result instanceof Blob) {
        downloadBlob(result, `page-1.${format}`);
        setDone(true);
      } else {
  ...
  return (
    <ToolLayout
      title="PDF to Image"
      description="Convert PDF pages to PNG or JPG. Single-page PDFs download directly; multi-page PDFs return a list."
      icon={<FileImage className="h-6 w-6" />}
    >
      {done && !multiResult ? (
        <DownloadSuccess filename={`page-1.${format}`} onReset={reset} />
      ) : done && multiResult ? (
        <div className="space-y-4 animate-slide-up">
          <div className="rounded-2xl border border-green-200 bg-green-50 p-4">
            <p className="font-semibold text-green-800">
              {multiResult.pageCount} pages converted to {multiResult.format.toUpperCase()} at {multiResult.dpi} DPI
            </p>
        ...
        <>
          <FileDropzone file={file} onFiles={(f) => { setFile(f[0]); setError(null); setDone(false); setMultiResult(null); }} />
              ...
              <div className="flex gap-2">
                {(['png', 'jpg'] as const).map((f) => (
                  <button
                    ...
                    onClick={() => setFormat(f)}
                    className={cn(
                      'flex-1 rounded-xl border-2 py-2 text-sm font-semibold uppercase transition-all',
                      format === f ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-600 hover:border-blue-300'
                    )}
                  >
                    {f}
                  </button>
