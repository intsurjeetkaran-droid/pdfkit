...
import { removeMetadata } from '@/lib/api';
...
export default function RemoveMetadataPage() {
  ...
  return (
    <ToolLayout title="Remove Metadata" description="Strip all embedded metadata from a PDF — title, author, creation date, XMP streams, and more." icon={<ShieldOff className="h-6 w-6" />}>
      {result ? (
        <DownloadSuccess blob={result} filename="clean.pdf" onReset={reset} />
      ) : (
        <>
          ...
          <Button fullWidth size="lg" sticky loading={loading} disabled={!file} onClick={handleRemove}>
