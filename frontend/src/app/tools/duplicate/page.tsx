...
import { organizeDuplicate, downloadBlob } from '@/lib/api';
...
export default function DuplicatePage() {
  ...
  async function handleDuplicate() {
    ...
    try {
      const blob = await organizeDuplicate(file, pages);
  ...
  return (
    <ToolLayout
      ...
      description="Duplicate specific pages. Each duplicate is inserted right after the original page."
      icon={<Copy className="h-6 w-6" />}
    >
      {done ? (
        <DownloadSuccess filename="duplicated.pdf" onReset={reset} />
      ) : (
        <>
          ...
          <Input
            ...
            hint="1-indexed page numbers, comma-separated. Each duplicate is inserted after the original."
          />
