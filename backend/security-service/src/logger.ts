...
const devFormat = winston.format.combine(
  ...
  winston.format.printf(({ timestamp, level, message, ...meta }) => {
    ...
    return `${timestamp} [security-service] ${level}: ${message}${metaStr}`;
  })
);
...
const logger = winston.createLogger({
  ...
  transports: [
    ...
    new winston.transports.File({
      filename: path.join(logsDir, 'security-service-error.log'),
      level: 'error', format: jsonFormat, maxsize: 10 * 1024 * 1024, maxFiles: 5,
    }),
    new winston.transports.File({
      filename: path.join(logsDir, 'security-service-combined.log'),
