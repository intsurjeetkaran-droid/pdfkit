...
export default function WatermarkPage() {
  ...
  async function handleWatermark() {
    ...
    try {
      setResult(await addWatermark(file, { text, opacity, rotation, fontSize }));
    } catch (err) {
  ...
  return (
    <ToolLayout title="Watermark PDF" description="Add a text watermark to every page of your PDF." icon={<Droplets className="h-6 w-6" />}>
      {result ? (
        <DownloadSuccess blob={result} filename="watermarked.pdf" onReset={reset} />
      ) : (
        <>
          <FileDropzone file={file} onFiles={(f) => { setFile(f[0]); setError(null); setResult(null); }} />
          <Input label="Watermark text" value={text} onChange={(e) => setText(e.target.value)} placeholder="e.g. CONFIDENTIAL" />
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={label} className="space-y-2">
                ...
                <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(parseFloat(e.target.value))} className="w-full" aria-label={label} />
              </div>
            ))}
          </div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 flex items-center justify-center min-h-[80px] overflow-hidden">
            <span className="font-bold text-slate-400 pointer-events-none select-none" style={{ fontSize: `${Math.min(fontSize * 0.5, 36)}px`, opacity, transform: `rotate(-${rotation}deg)`, display: 'inline-block' }}>
              {text || 'Preview'}
            </span>
          </div>
          {error && <Alert type="error" message={error} onDismiss={() => setError(null)} />}
          <Button fullWidth size="lg" sticky loading={loading} disabled={!file} onClick={handleWatermark}>
